...
const CriarQuestionarioModal = ({ visible, onClose, onSave }) => {
  ...
  const handleAdicionarPergunta = () => {
    // Fecha a última pergunta aberta
    const perguntasAtualizadas = perguntas.map((p, idx) =>
      idx === perguntas.length - 1 ? { ...p, aberta: false } : p
    )
    setPerguntas([
  ...
  // Alterna collapse da pergunta
  const handleTogglePergunta = (id) => {
    setPerguntas(perguntas.map(p => p.id === id ? { ...p, aberta: !p.aberta } : p))
  }
